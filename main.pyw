...
class Principal(QMainWindow):
    def __init__(self):
        ...
        self.sbFuente.valueChanged.connect(self.leerVersiculo)
    ...
    def datosMostrar(self):
        self.mostrar.textEdit.setStyleSheet("background-color: rgb(%s); color: rgb(%s)" % (",".join(self.data["colorFondo"]), ",".join(self.data["colorFuente"])))
    ...
    def seleccionarColorFuente(self):
        ...
        modificarData("colorFuente", [str(color.red()), str(color.green()), str(color.blue()), str(color.alpha())])
        self.data=leerData()
        self.datosMostrar()
    ...
    def seleccionarColorFondo(self):
        ...
        modificarData("colorFondo", [str(color.red()), str(color.green()), str(color.blue()), str(color.alpha())])
        self.data=leerData()
        self.datosMostrar()
    ...
    def cargarDatos(self):
        ...
        if "cbHistorial" in self.data:
            self.cbHistorial.clear()
            for dato in self.data["cbHistorial"]:
                self.cbHistorial.addItem(dato)
            self.cbHistorial.setCurrentIndex(-1)
        else:
            modificarData("cbHistorial", [])
    ...
    def leerVersiculo(self):
        ...
        for libro in self.biblia:
            if libro["nombre"]==self.cbLibro.currentText():
                ...
                self.tePrev.insertHtml('<p style="font-size:%ipx;"><strong>%s %i:%i</strong> %s</p>' % (self.sbFuente.value(), self.cbLibro.currentText(), capitulo+1, versiculo+1, libro["capitulos"][capitulo][versiculo]))

                if verMenos1 in range(self.cbVersiculo.count()-1): self.tePrevMenos1.insertHtml('<p style="font-size:%ipx;"><strong>%s %i:%i</strong> %s</p>' % (self.sbFuente.value(), self.cbLibro.currentText(), capitulo+1, verMenos1+1, libro["capitulos"][capitulo][verMenos1]))

                if verMenos2 in range(self.cbVersiculo.count()-1): self.tePrevMenos2.insertHtml('<p style="font-size:%ipx;"><strong>%s %i:%i</strong> %s</p>' % (self.sbFuente.value(), self.cbLibro.currentText(), capitulo+1, verMenos2+1, libro["capitulos"][capitulo][verMenos2]))

                if verMas1 in range(self.cbVersiculo.count()-1): self.tePrevMas1.insertHtml('<p style="font-size:%ipx;"><strong>%s %i:%i</strong> %s</p>' % (self.sbFuente.value(), self.cbLibro.currentText(), capitulo+1, verMas1+1, libro["capitulos"][capitulo][verMas1]))

                if verMas2 in range(self.cbVersiculo.count()-1): self.tePrevMas2.insertHtml('<p style="font-size:%ipx;"><strong>%s %i:%i</strong> %s</p>' % (self.sbFuente.value(), self.cbLibro.currentText(), capitulo+1, verMas2+1, libro["capitulos"][capitulo][verMas2]))
